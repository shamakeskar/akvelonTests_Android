package com.linkedin.android.screens;
...
public class ScreenLogin extends BaseScreen {
    ...
    public void verify() {
        ...
        HardwareActions.takeCurrentActivityScreenshot("Login screen");
    }
    ...
    public void waitForSyncContactsDialog() {
        ...
        Assert.assertTrue ("'Sync with existing contacts' text is not present", 
                getSolo().searchText("Sync with existing contacts"));
        Assert.assertTrue ("'Do not sync LinkedIn contacts' text is not present", 
                getSolo().searchText("Do not sync LinkedIn contacts"));
    }
    /**
     * Default handler.
     * Taps on 'Do not sync LinkedIn contacts' text on Sync Contacts Dialog.
     */
    ...
    public boolean isCalendarSplashPresents() {
        return getSolo().waitForActivity(CALENDAR_SPLASH_ACTIVITY_SHORT_CLASSNAME, DataProvider.WAIT_DELAY_DEFAULT);
    }
    ...
    public void waitForCalendarSplashScreen() {
        Assert.assertTrue("Cannot wait Calendar Splach activity",
                getSolo().waitForActivity(CALENDAR_SPLASH_ACTIVITY_SHORT_CLASSNAME, DataProvider.WAIT_DELAY_DEFAULT));
        ...
        Assert.assertTrue("'Add Calendar' text is not present", 
                getSolo().searchText("Add Calendar"));
        Assert.assertTrue("'Learn More' text is not present", 
    ...
    public void verifyLearnMoreSplashScreen() {
        Assert.assertTrue("'How LinkedIn uses your calendar data' text is not present", getSolo().waitForText("How LinkedIn uses your calendar data"));
        
        Assert.assertTrue("'Learn More' text is not present", getSolo().searchText("Learn More"));
        Assert.assertNotNull("'This feature accesses your phone's calendar...' text is not present", ViewUtils.searchTextViewInLayout("This feature accesses your phone's calendar", null, false));
    }
}
