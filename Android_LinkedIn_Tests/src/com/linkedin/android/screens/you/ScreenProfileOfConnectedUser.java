...
public class ScreenProfileOfConnectedUser extends ScreenProfile {
    ...
    // CONSTRUCTORS ---------------------------------------------------------
    public ScreenProfileOfConnectedUser() {
        super();
    }
    ...
    public void verify() {
        ...
        ViewAssertUtils.assertViewIsPlacedInLayout("'Call' button is not present", callButton,
                CALL_BUTTON_LAYOUT);

        ImageButton sendMessageButton = getSendMessageButton();
        ViewAssertUtils.assertViewIsPlacedInLayout("'Send message' button is not present",
                sendMessageButton, SEND_MESSAGE_BUTTON_LAYOUT);

        ImageButton forwardButton = getForwardButton();
        ViewAssertUtils.assertViewIsPlacedInLayout("'Forward' button is not present",
                forwardButton, FORWARD_BUTTON_LAYOUT);

        Assert.assertTrue(
                "'Call', 'Send message' and 'Forward' buttons are not placed 'in-line' horizontally",
